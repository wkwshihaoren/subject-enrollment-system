...
Fundamentals of Software Development - Autumn 2026 Assignment Part 2: Software Development
Group members: Jingxiang Wang, Jason Maharjan, Miki Hayashi, Aditya Maurya.
...
For running the CLI, use the command:

```bash
python cli_main.py
```

For running the GUI, use the command:
...
Use custom `c_print` and `c_input` functions to use coloured prints and inputs according to the required log type, and indentation levels. Example:
```bash
